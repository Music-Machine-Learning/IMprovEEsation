...
int compute_similarity(struct piece_s * ggoal, struct piece_s * gtrial, uint8_t * sim){
	...
	*sim = ((csim * 100) / ctot);
	return ((csim * 100) / ctot);
}

/* Functions which sort the pool according to similarity (with mergesort) */
void merge_pool(struct piece_s * pool [GENETIC_POOL_SIZE], uint8_t * sim, int fst, int q, int end){
...
void mergesort_pool(struct piece_s * pool[GENETIC_POOL_SIZE], uint8_t * sim, int fst, int end){
	int q;
	if (fst < end){
		q = ((fst + end) / 2);
...
int sort_pool(struct piece_s * pool[GENETIC_POOL_SIZE], uint8_t * sim){
